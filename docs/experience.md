...
`scripts/append_findings --render-only` regenerates the tally from the episodes,
...
is generated from the episodes' `Kind` fields by
`scripts/append_findings --render-only`; regenerate it after adding an episode.
