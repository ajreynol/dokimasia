...
| `regression_audit/todo.md` | candidates for removing tester directives and required validation |
...
what this repository is for, and we have produced none.** One finding is filed — `f-1` / `tcb-001`
below, a kind C refactoring ask. The accurate summary is **instrumentation that works,
...
| ◐ | `API` | **proof API contracts** | do literal proof-step lists agree with checker assertions? does a `ProofGenerator` return a proof of what was asked? |
...
| `INFER` | `dokimasia_analyzer.preprocess` | 2 explicit null-generator skolem lemma sites on local preprocessing paths: sets and bags | `i-24`; static candidates only |
| `API` | `dokimasia_analyzer.proofshape` | 470 of 570 literal-rule calls compared on at least one list; 0 arity disagreements; 100 calls unknown | entry assertions recovered for 117 rules; no runtime claim |
...
| `API` | literal call arities are checked; receiver types, dynamic vectors and general generator contracts still need the AST tier |
...
| `INFER0001` | a local `ppRewrite` path pushes a `SkolemLemma` made with `mkTrustLemma(..., nullptr)` | inline or adjacent bindings only; methods with proof handling are conservatively skipped; gates and final-proof reachability are untested |
| `INFER0002` | an emitted inference lacks a switch case in its theory's reconstructor, whose fallback constructs trust | call-site generators and reachability are not resolved |
| `API0005` | a literal `addStep` or proof `mkNode` list violates a leading checker arity assertion | no receiver type resolution; dynamic lists, combined/switch dispatch and ambiguous contracts remain unknown; assertions can compile out |
...
| **i-24** | **`set.choose`'s reduction lemma is built with no proof generator, and `set.choose` is ungated in safe mode.** `TheorySetsPrivate::expandChooseOperator` (`theory/sets/theory_sets_private.cpp:1716`) pushes its skolem lemma — `(and (= x (@sets_choose A)) (or (= A set.empty) (set.member x A)))` — as `TrustNode::mkTrustLemma(lem, nullptr)`. `TheoryEngine::ppRewrite` (`theory/theory_engine.cpp:966`) sees the null generator and stamps `TrustId::THEORY_PREPROCESS_LEMMA`, which `EoPrinter::isHandled` refuses. **The companion `mkTrustRewrite(node, x, nullptr)` is not the problem**: `x` is a purify skolem, so `expandMacros` discharges that step through `applyPredIntro` modulo original forms (`smt/proof_post_processor.cpp:190`) — the same discharge strings relies on at `theory_strings.cpp:1166`, where the lemma itself is `eagerReduceTrusted` and *does* carry a generator. **Sets is missing a branch its sibling has.** `OperatorElim::mkSkolemLemma` (`theory/arith/operator_elim.cpp:501`) is the same construction done correctly: `mkTrustLemma(lem, this)` when `d_env.isTheoryProofProducing()`, `nullptr` only when proofs are off. `expandChooseOperator` passes `nullptr` unconditionally — as does `TheoryBags::ppRewrite` (`theory/bags/theory_bags.cpp:170`) for `bag.choose`, the same lemma shape, though bags is swept out of safe mode and sets is not. So this is not a design gap; it is one missing conditional. The gate is open: `TheorySets::ppRewrite` gates only `SET_UNIVERSE`, `SET_COMPLEMENT`, `RELATION_JOIN_IMAGE` and `SET_COMPREHENSION`, and `SET_CHOOSE` is one of the 17 sets kinds our own fragment report leaves available (`bug_db/fragment.md`). Distinct from `i-22`: that is an `InferProofCons` switch fall-through, this is a preprocessing lemma that never had a generator at all | `preprocess check` (`INFER0001`) | **an input, written but not yet run.** Two candidates, each needing one half of the lemma. The `or` half: `(declare-fun s () (Set Int))`, `(assert (not (= s (as set.empty (Set Int)))))`, `(assert (not (set.member (set.choose s) s)))`. The `(= x (@sets_choose A))` half, through congruence, with `f` indirecting so that `TheorySets::ppAssert` cannot solve the equality away: `(declare-fun f (Int) (Set Int))` over Int constants `a`, `b`, then `(assert (= a b))` and `(assert (distinct (set.choose (f a)) (set.choose (f b))))`. Both `unsat`; run each under `--safe-mode=safe --produce-proofs --check-proofs`, which implies `--check-proofs-complete` (`i-3`). Rank 1 if it reports *"The proof was incomplete due to a trust step with id THEORY_PREPROCESS_LEMMA"*; spurious in the manner of `s-6` if it comes back clean — the untested step is whether the lemma survives preprocessing into the final proof. Note the fix has two halves: even with a generator, `EoNodeConverter::isHandledSkolemId` (`proof/eo/eo_node_converter.cpp:682`) still refuses the `SETS_CHOOSE` skolem (`i-19`) |
...
they check.** cvc5's internal proof checker is the natural trusted kernel, and
its value depends on how little of cvc5 it needs to be right. The base class is
...
| --- | --- | --- | --- |
| `theory/strings/proof_checker.cpp` | `theory/strings/core_solver.h` | 3,193 lines | used |
| `theory/arith/proof_checker.cpp` | `theory/arith/linear/constraint.h` | 2,556 lines | **unused** |
| `theory/datatypes/proof_checker.cpp` | `theory_datatypes_utils.h` | 1,027 lines | used |
| `theory/arith/nl/transcendental/proof_checker.cpp` | `sine_solver.h` | 809 lines | used |
| `theory/datatypes/proof_checker.cpp` | `expr/dtype_cons.h` | 479 lines | **unused** |
| `theory/builtin/proof_checker.cpp` | `theory/quantifiers/extended_rewrite.h` | 270 lines | **unused** |

Where the edge is `used`, the helpers the checker wants are `static` methods
parked on solver classes — pure functions of `NodeManager*` and `Node` that do
not need the solver — and C++ makes you include the whole class to reach one.
...
| not yet — **run-it** | `i-1`, `i-22`, `i-6`, `i-24` | a static argument with no input. `i-1` has had one attempt fail; `i-24` has its inputs written and unrun |
...
the coupling. Written up as `f-1` in the filed section above.
