...
| **i-4** | **Completeness depends on a search budget.** `proof_post_processor_dsl.cpp` reconstructs under `--proof-rewrite-rcons-rec-limit` (default 5); when the search fails the macro step stays and the proof is incomplete. **38 macro rewrite gaps** ride on this | `rewrites gaps` | a maintainer's view on whether the budget can be exceeded in practice. Bounds what any kernel contract can claim |
