...
| i-20 | **The `\inferrule` documentation is prose, not a specification.** Comparing it against the signature produced 72 apparent disagreements, of which every one inspected was our parser meeting another LaTeX convention. What remains is the printer's argument *reshaping*, which we do not model. cvc5 states a rule's arity in LaTeX only — unlike `SkolemId`, which states "Number of skolem indices: N" and is checkable | `signature arity` |
