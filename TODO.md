...
The goal is **complete proofs, always** (`docs/README.md`).

**This file holds work *we* do**, and it is meant to stay short. Anything we ask
cvc5 to act on is in `docs/README.md`; the reasoning behind any
of it is in `docs/`; the catalogue of checks is
`docs/README.md`. A numbered section in a design note is a
...
in the tree: the promises (`dokimasia/README.md`), each with the tier
and the test behind it, and the bar (`dokimasia/README.md`).
...
| **of that inventory, what any input has reached** | **0 in safe mode, 21 of 203 unrestricted (`docs/maintenance.md`)** | the latent set, named |
...
| rule checkers over-scoped by a solver include | **6** (`tcb-001` (`docs/experience.md`)) | 0 |

**Left to do:** nothing queued. The kernel obligations
(`docs/README.md`) stay parked behind `i-4` — reconstruction
...
withdrawn (`docs/README.md`) — it would have published cvc5's internal
...
| metric | today | target |
| --- | --- | --- |
| options declaring no proof support that safe mode leaves on | **1** (`stringLazyPreproc`; `macrosQuantMode` is spurious (`docs/README.md`)) | 0 |
| completeness chain links that hold | **4 of 5** — and the fifth cannot be stated (`docs/README.md`) in safe mode at all | 5, asserted |
...
| **holes reached in safe mode over `regress0`** | **0, over 1,061 proofs (`docs/maintenance.md`)** | stays 0 |
...
thing we would report (`docs/README.md`)), **R5** (`no_support` covers
...
- **A design question is answered with a check.** When a cvc5 developer asks
  *why is this the way it is*, the deliverable is an invariant, a verifier, and
  ...
  standing decision and the register are `docs/experience.md`.
...
- **A finding is confirmed before it is filed**, and for a defect that means an
  input (`docs/experience.md` (`dokimasia/README.md`)).
...
- **We never open a pull request or an issue.**
  `docs/experience.md#the-bar` (`dokimasia/README.md`).
...
- **Fuzzing.** murxla's job (`docs/maintenance.md`). What we can
...
- **A diagnostic/SARIF framework and a generated check registry.** Modelled on
  anoieu and never needed: `docs/README.md` is written by hand and is accurate,
