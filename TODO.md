...
| **c-17** | **Safe mode is stricter at the Eunoia seam than the default.** `isHandledTheoryRewrite` accepts `ARITH_POW_ELIM`, `ARRAYS_SELECT_CONST` and `LAMBDA_ELIM` only when `safeMode == UNRESTRICTED`; all three are implemented by a rewriter. If one is applied under `--safe-mode=safe`, the step cannot be printed | `rewrites gaps` ✅ | 2 | probably gated: lambdas need `ufHoExp`, `POW` needs `arithExp`, constant arrays need `arraysExp` — all disabled by safe mode. **Needs the option gate to settle**, which is exactly the machinery we do not have |
| c-18 | **2 hard rewrite gaps**: `ARRAYS_EQ_RANGE_EXPAND` and `DT_MATCH_ELIM` are applied by a rewriter, refused by the seam, and have no macro to expand into | `rewrites gaps` ✅ | 3 | arrays `eqrange` and datatypes `match` are both plausibly expert-gated; same caveat |
