...
`python3 -m dokimasia_analyzer check <cvc5>` runs all eight ratchets in a single
...
def _run(mod: str, argv: list[str]) -> tuple[int, str]:
    ...
    m = importlib.import_module(f".{mod}.__main__", package="dokimasia_analyzer")
    buf = io.StringIO()
    try:
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="python3 -m dokimasia_analyzer",
